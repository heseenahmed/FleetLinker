...
namespace FleetLinker.Application.Queries.Equipment.EquipmentRequest
{
    ...
    public record GetMechanicalMaintenanceRequestsQuery(string MechanicalId) : IRequest<APIResponse<List<EquipmentRequestDto>>>;

    public record GetOwnerMaintenanceRequestsQuery(string OwnerId) : IRequest<APIResponse<List<EquipmentRequestDto>>>;
}
